...
const server = http.createServer(async (req, res) => {
  ...
  // Handle Telegram notification API endpoint
    ...
    req.on('end', async () => {
      try {
        ...
        if (!BOT_TOKEN || !CHAT_ID) {
          console.error('❌ Server Error: TELEGRAM_BOT_TOKEN or TELEGRAM_CHAT_ID is missing in env');
          res.writeHead(500, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ error: 'Server configuration error: missing env variables' }));
          return;
        }

        const tgUrl = https://api.telegram.org/bot/sendMessage;
...
server.listen(PORT, () => {
  console.log(🚀 Financial Goal Calculator Server running on port );
});
